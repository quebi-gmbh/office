/**
 * Paint toolbar — tool picker, size slider, colour controls, undo/redo, download.
...
export function Toolbar({ engine, state }: ToolbarProps) {
  function download() {
    ...
    a.href = url;
    const now = new Date();
    const ts = now.toISOString().slice(0, 16).replace(/[-T:]/g, m => m === "T" ? "-" : m).replace(":", "");
    a.download = `paint-${ts}.png`;
    a.click();
  }

  function clear() {
    engine.newDocument(state.doc.width, state.doc.height, state.doc.bgWasTransparent ? "transparent" : state.bg);
  }
  ...
  return (
    <header className="paint-toolbar">
      ...
      {/* Eraser mode (only shown when eraser active) */}
      {state.tool === "eraser" && (
